...
a,b,c = np.polyfit(v,e,2) # fit a parabola to the data (simplest fit)
...
# Birch Murnaghan equation of state function
def birch_murnaghan(params,vol):
	E0, B0, BP, V0 = params
	eta = (V0/vol)**(2/3)
	return (E0 + (9*V0*B0/16)*(((eta - 1)**3)*BP + ((eta - 1)**2)*(6 - 4*eta)))
    
# Murnaghan equation of state function
...
# If you don't have pylab, use: pip3 install pylab
try:
	import pylab as pl
except ImportError:
	sys.stderr.write('pylab module not available, skipping plot (use pip to install pylab if you want to plot)')
	sys.exit(0)
